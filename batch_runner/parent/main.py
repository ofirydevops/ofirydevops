...
import subprocess
import batch_runner.parent.validation as validation
...
def create_batch_job_defenition(batch_conf, 
    ...
    response = batch.register_job_definition(
        ...
        containerProperties  = {
            ...
            'command': ['pipenv', 'run', 'python3.10', '-m', 'batch_runner.child.main'],
            'volumes': [
                { 
...
async def run_batch(name, batch_env, child_job_image, child_inputs, child_job_entrypoint):
    ...
    batch_conf_str   = utils.get_ssm_param(f"{validation.BATCH_ENV_SSM_PARAM_PREFIX}{batch_env}") 
